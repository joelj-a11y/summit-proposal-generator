    ...
    try:
        ...
        if services.get('includePermitting'):
            ...
            if len(swmp_rows) > 3:
                ...
                if len(cells) >= 5:
                    set_cell_text(cells[2], '1')  # Quantity
                    set_cell_text(cells[3], f'${permitting_price:,.0f}')  # Rate
                    set_cell_text(cells[4], f'${permitting_price:,.0f}')  # Total
        ...
        if routine and len(inspection_rows) > 1:
            ...
            if len(cells) >= 5:
                ...
                set_cell_text(cells[3], f"${routine.get('rate', 0):,.0f}")
                set_cell_text(cells[4], f"${routine.get('total', 0):,.0f}")
        ...
        if storm and len(inspection_rows) > 2:
            ...
            if len(cells) >= 5:
                ...
                set_cell_text(cells[3], f"${storm.get('rate', 0):,.0f}")
                set_cell_text(cells[4], f"${storm.get('total', 0):,.0f}")
        ...
        if post and len(inspection_rows) > 3:
            ...
            if len(cells) >= 5:
                ...
                set_cell_text(cells[3], f"${post.get('rate', 0):,.0f}")
                set_cell_text(cells[4], f"${post.get('total', 0):,.0f}")
        ...
        if len(inspection_rows) > 4:
            ...
            if len(cells) >= 5:
                set_cell_text(cells[4], f"${inspection_subtotal:,.0f}")
        
        # TABLE 4: Other Services (Items 4, 5)
        # Could add MAR here if needed
        ...
        if len(total_rows) > 1:
            ...
            if len(cells) >= 3:
                ...
                set_cell_text(cells[2], f"${project_total:,.0f}")
        ...
        if services.get('includePermitting'):
            ...
            if permitting_description:
                # Find table 2, row 3 (1C) and update description
                swmp_table = tables[1]
                swmp_rows = swmp_table.getElementsByTagName('w:tr')
                if len(swmp_rows) > 3:
                    cells = swmp_rows[3].getElementsByTagName('w:tc')
                    if len(cells) >= 2:
                        # Append description to existing text
                        desc_cell = cells[1]
                        paras = desc_cell.getElementsByTagName('w:p')
                        if paras:
                            para = paras[0]
                            run = doc.createElement('w:r')
                            t = doc.createElement('w:t')
                            t.setAttribute('xml:space', 'preserve')
                            t.appendChild(doc.createTextNode(f" {permitting_description}"))
                            run.appendChild(t)
                            para.appendChild(run)

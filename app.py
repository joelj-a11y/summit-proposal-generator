...
def webhook():
    """Receive quote data and log to Google Sheets"""
    try:
        ...
        row_data = [
            ...
            data.get('projectCity', ''),
            data.get('projectCounty', ''),
            str(data.get('disturbedAcres', '')),
            data.get('nearestInspector', ''),
            str(data.get('calculatedPrice', '')),
            str(data.get('rateSheetPrice', '')),
            str(data.get('difference', '')),
            str(data.get('percentDiff', ''))
        ]
        ...
        result = service.spreadsheets().values().append(
            ...
            range='Sheet1!A:K',  # Adjust range if your sheet has a different name
            valueInputOption='RAW',
            body=body
        ).execute()
        
        print(f"Successfully added row to Google Sheets: {result}")
        
        return jsonify({
            'success': True,
            'message': 'Quote logged to Google Sheets',

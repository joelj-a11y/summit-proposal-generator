...
def generate_proposal_docx(quote_data):
    ...
    try:
        ...
        if len(tables) < 2:
            raise Exception(f"Template needs at least 2 tables, found only {len(tables)}")
        ...
        if len(info_rows) < 5:
            raise Exception(f"Table 1 needs at least 5 rows, found only {len(info_rows)}")
        
        # Row 0: Client, Project
        cells = info_rows[0].getElementsByTagName('w:tc')
        print(f"DEBUG: Row 0 has {len(cells)} cells")
        if len(cells) >= 4:
        ...
        print(f"DEBUG: Table 2 (pricing) has {len(pricing_rows)} rows")
        
        if len(pricing_rows) < 8:
            raise Exception(f"Table 2 needs at least 8 rows for services, found only {len(pricing_rows)}")
        
        services = quote_data.get('services', {})
        
        # Item 3: Permitting (Row 3 in pricing table)
        ...
        # Item 4: Routine (Row 4)
        routine = services.get('routine', {})
        if routine and len(pricing_rows) > 4:
        ...
        # Item 5: Post-Storm (Row 5)
        storm = services.get('postStorm', {})
        if storm and len(pricing_rows) > 5:
        ...
        # Item 6: Post-Construction (Row 6)
        post = services.get('postConstruction', {})
        if post and len(pricing_rows) > 6:
        ...
        # Item 7: MAR (Row 7)
        if services.get('includeMAR') and len(pricing_rows) > 7:
